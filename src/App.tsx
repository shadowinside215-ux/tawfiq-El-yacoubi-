...
function MainApp() {
  ...
  return (
    <div className={`min-h-screen bg-brand-dark text-white font-sans selection:bg-brand-accent selection:text-white ${isRtl ? 'font-arabic' : ''}`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      {/* Trophy Ticker - Very Top */}
      <div className="fixed top-0 left-0 right-0 bg-brand-red py-2 overflow-hidden border-b border-white/10 z-[60]">
        <div className="flex whitespace-nowrap animate-marquee">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex items-center gap-12 px-6">
              {Object.entries(t.achievements).filter(([key]) => key !== 'title' && key !== 'certified' && key !== 'coaching_since').map(([key, value]) => (
                <div key={key} className="flex items-center gap-3 text-white">
                  <Trophy size={14} className="text-brand-gold shrink-0" />
                  <span className="text-[10px] font-black uppercase tracking-widest">{value as string}</span>
                </div>
              ))}
            </div>
          ))}
      ...
      {/* Navbar */}
      <nav className={`fixed top-12 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-brand-dark/95 border-b border-brand-accent/20 py-3 shadow-2xl shadow-brand-accent/5 top-0' : 'bg-transparent py-6'}`}>
      ...
      {/* why green-dark was replaced in css, checking App.tsx usages */}
      {/* My Story Section */}
      <section id="story" className="py-24 bg-brand-red-dark/30">
      ...
      {/* Why Train With Me */}
      <section className="py-24 bg-brand-red-dark">
        <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="glass p-12 rounded-none max-w-2xl mx-auto border-l-4 border-brand-accent">
            ...
            <a 
              ...
              className="inline-flex items-center gap-4 px-12 py-6 bg-brand-red text-white text-xl font-bold rounded-none hover:scale-105 transition-all shadow-2xl shadow-brand-red/20 uppercase tracking-tighter"
            >
              {t.contact.ready}
            </a>
          </div>
